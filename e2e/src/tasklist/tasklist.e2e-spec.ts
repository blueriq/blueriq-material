...
describe('TaskList App', () => {
  ...
  describe('tasklist', () => {
    ...
    it('should handle task and case events', () => {
      ...
      // switch to the first tab with a tasklist
      app.switchToTaskListOneTab();
      app.verifyOnTaskListPage();

      // tasklist should contain 2 tasks
      app.verifyNumberOfTasks(2);

      // start task 'Toevoegen bewijsstukken'
      app.startTask('Toevoegen bewijsstukken');

      // verify that other tasklist buttons are now disabled
      ...
      // back to case overview with tasklist again
      app.verifyOnTaskListPage();

      // verify that other tasklist buttons are enabled again
      app.switchToTaskListTwoTab();
      app.verifyOnTaskListPage();
